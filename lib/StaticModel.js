		...
		, __loadModel: function( record, callback ){
			var keys, k, rKeys, r, relations, waiter, instance, opts = {
		 	...
		 	if ( this.isDistributed() ){
		 		...
		 		if ( debug ) log.debug( "[staticmodel] sending cache message for [dmodel-" + this.__database + "/" + this.__model + "@" + instance.id + "], action [init]: " ), log.dir( instance.getValues() );
			...
			while( i-- ){
				if ( keys[ i ] === "$limit" ){
					...
					config[ keys[ i ] ].forEach( function( item ){
						...
						if ( m === 0 ) str += " (";
						if ( m > 0 && m < len ) str += " OR ";
						str += this.__db.escapeField( keys[ i ] ) + this.__getOperator( item );
						if ( m + 1 === len ) str += " )";
						queries.push( str );
					...
					queries.push( this.__db.escapeField( keys[ i ] ) + this.__getOperator( config[ keys[ i ] ] ) );
